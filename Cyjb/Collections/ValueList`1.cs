...
/// <remarks><para>适合临时使用列表的场景，使用栈上分配的数组来避免内存回收；也支持利用
/// <see cref="ArrayPool{T}"/> 自动扩容。</para></remarks>
public ref struct ValueList<T>
{
	...
	public Span<T> AsSpan() => span[0..length];
	...
	{
		if (span[0..length].TryCopyTo(destination))
		{
			itemsWritten = length;
			return true;
		}
