...
namespace Cyjb
{
	...
	public sealed class PowerBinder : Binder
	{
			...
			ParameterModifier[] modifiers, CultureInfo culture, string[] names, out object state)
		{
			...
			for (int i = 0; i < match.Length; i++)
			{
				if (match[i] != null)
				{
					...
					if (names == null)
					{
						info.ParamOrder = GetParamOrder(len);
					}
					else
					{
		...
		private MatchInfo SelectMethod(BindingFlags bindingAttr, MatchInfo[] infos, int len, Type[] types)
		{
			...
			for (int i = 0; i < len; i++)
			{
				if (MakeGenericMethod(infos[i], types) && CheckMethodParameters(infos[i], types, optionalParamBinding))
				{
					infos[idx++] = infos[i];
				}
			}
		...
		/// <returns>如果可以进行类型转换，则为 <c>true</c>；否则为 <c>false</c>。</returns>
		private bool CanChangeType(Type type, Type fromType)
		{
			if (fromType == null)
			{
				return type.IsClass;
		...
		private int FindMostSpecific(MatchInfo match1, MatchInfo match2, Type[] types)
		{
			...
			{
				switch (FindMostSpecificType(match1.Parameters[i].ParameterType, match2.Parameters[i].ParameterType, types[i]))
				{
					case 1:
						p1Better = true;
						break;
