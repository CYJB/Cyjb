...
namespace TestCyjb.Collections.TestCharSet
{
	...
	/// </summary>
	/// <remarks><see cref="OldBitCharSet"/> 类采用类似位示图的树状位压缩数组判断字符是否存在，
	...
	public sealed class OldBitCharSet : SetBase<char>, ISerializable
	{
		...
		/// <summary>
		/// 初始化 <see cref="OldBitCharSet"/> 类的新实例。
		/// </summary>
		/// <overloads>
		/// <summary>
		/// 初始化 <see cref="OldBitCharSet"/> 类的新实例。
		/// </summary>
		/// </overloads>
		public OldBitCharSet()
			: this(false, null)
		{ }
		/// <summary>
		/// 使用指定的是否区分大小写初始化 <see cref="OldBitCharSet"/> 类的新实例。
		/// </summary>
		/// <param name="ignoreCase">是否不区分字符的大小写。</param>
		public OldBitCharSet(bool ignoreCase)
			: this(ignoreCase, null)
		{ }
		/// <summary>
		/// 使用指定的是否区分大小写和区域信息初始化 <see cref="OldBitCharSet"/> 类的新实例。
		/// </summary>
		/// <param name="ignoreCase">是否不区分字符的大小写。</param>
		/// <param name="culture">不区分字符大小写时使用的区域信息。</param>
		public OldBitCharSet(bool ignoreCase, CultureInfo culture)
		{
		...
		/// <summary>
		/// 初始化 <see cref="OldBitCharSet"/> 类的新实例，该实例包含从指定的集合复制的元素。
		/// </summary>
		...
		/// <summary>
		/// 用指定的序列化信息和上下文初始化 <see cref="OldBitCharSet"/> 类的新实例。
		/// </summary>
		/// <param name="info"><see cref="SerializationInfo"/> 对象，包含序列化 
		/// <see cref="OldBitCharSet"/> 所需的信息。</param>
		/// <param name="context"><see cref="StreamingContext"/> 对象，
		/// 该对象包含与 <see cref="OldBitCharSet"/> 相关联的序列化流的源和目标。</param>
			...
			bool returnIfUnfound, out int sameCount, out int unfoundCount)
		{
			...
			OldBitCharSet uniqueSet = new OldBitCharSet(ignoreCase, culture);
		...
		private bool ContainsAllElements(OldBitCharSet other)
		{
			Contract.Requires(other != null);
		...
		/// <summary>
		/// 将 <see cref="OldBitCharSet"/> 对象的容量设置为它所包含
		...
		/// </summary>
		/// <param name="other">要从集内移除的项的集合。</param>
		private void ExceptWith(OldBitCharSet other)
		{
			Contract.Requires(other != null);
			for (int i = 0; i < TopLen; i++)
			{
		...
		public override void IntersectWith(IEnumerable<char> other)
		{
			...
			{
				otherSet = new OldBitCharSet(other.Where(this.Contains), ignoreCase, culture);
				// 替换当前集合。
		...
		/// </summary>
		/// <param name="other">要与当前集进行比较的集合。</param>
		private void IntersectWith(OldBitCharSet other)
		{
			Contract.Requires(other != null);
			// 针对 BitCharSet 的操作更快。
			for (int i = 0; i < TopLen; i++)
		...
		/// <returns>如果当前集与 <paramref name="other"/> 
		/// 至少共享一个通用元素，则为 <c>true</c>；否则为 <c>false</c>。</returns>
		private bool Overlaps(OldBitCharSet other)
		{
			Contract.Requires(other != null);
			for (int i = 0; i < TopLen; i++)
			{
		...
		public override void SymmetricExceptWith(IEnumerable<char> other)
		{
			...
			else
			{
				...
				if (otherSet == null ||
					...
					!Equals(this.culture, otherSet.culture))
				{
					otherSet = new OldBitCharSet(other, ignoreCase, culture);
				}
				// 针对 BitCharSet 的操作更快。
		...
		/// </summary>
		/// <param name="other">要与当前集进行比较的集合。</param>
		private void SymmetricExceptWith(OldBitCharSet other)
		{
			Contract.Requires(other != null);
			for (int i = 0; i < TopLen; i++)
			{
		...
		/// </summary>
		/// <param name="other">要与当前集进行比较的集合。</param>
		private void UnionWith(OldBitCharSet other)
		{
			Contract.Requires(other != null);
			for (int i = 0; i < TopLen; i++)
			{
		...
		/// <summary>
		/// 获取 <see cref="OldBitCharSet"/> 中包含的元素数。
		/// </summary>
		/// <value><see cref="OldBitCharSet"/> 中包含的元素数。</value>
		public override int Count
		{
			get { return this.count; }
		}
		/// <summary>
		/// 从 <see cref="OldBitCharSet"/> 中移除所有元素。
		/// </summary>
		...
		/// <summary>
		/// 确定 <see cref="OldBitCharSet"/> 是否包含特定值。
		/// </summary>
		/// <param name="item">要在 <see cref="OldBitCharSet"/> 
		/// 中定位的对象。</param>
		/// <returns>如果在 <see cref="OldBitCharSet"/> 
		...
		/// <summary>
		/// 从 <see cref="OldBitCharSet"/> 中移除特定对象的第一个匹配项。
		/// </summary>
		/// <param name="item">要从 <see cref="OldBitCharSet"/> 中移除的对象。</param>
		/// <returns>如果已从 <see cref="OldBitCharSet"/> 中成功移除 <paramref name="item"/>，
		/// 则为 <c>true</c>；否则为 <c>false</c>。如果在原始 <see cref="OldBitCharSet"/> 
